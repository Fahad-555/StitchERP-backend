...
public sealed class MySqlUserStore : IUserStore
{
    ...
    public string CreateVerificationToken(long id) { var token = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32)); using var command = Command("INSERT INTO email_verification_tokens (user_id, token_hash, expires_at) VALUES (@id, SHA2(@token, 256), DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 24 HOUR))"); Add(command, "@id", id); Add(command, "@token", token); command.ExecuteNonQuery(); return token; }
    public ManagedUser VerifyEmail(string token) { using var command = Command("UPDATE app_users u JOIN email_verification_tokens t ON t.user_id = u.user_id SET u.email_verified = 1, t.used_at = CURRENT_TIMESTAMP WHERE t.token_hash = SHA2(@token, 256) AND t.used_at IS NULL AND t.expires_at > CURRENT_TIMESTAMP"); Add(command, "@token", token); if(command.ExecuteNonQuery() != 1) throw new UnauthorizedAccessException("Email verification token is invalid or expired."); using var lookup = Command("SELECT u.user_id FROM app_users u JOIN email_verification_tokens t ON t.user_id=u.user_id WHERE t.token_hash=SHA2(@token,256)"); Add(lookup, "@token", token); return FindById(Convert.ToInt64(lookup.ExecuteScalar())); }
